...
async function cleanupExistingRecords() {
  console.log('Fetching existing records to clean up...');
  ...
  await base('Outcome-Worldview Relationships')
    ...
    .eachPage((records, fetchNextPage) => {
      records.forEach((record) => {
        existingRecords.push(record.id);
      });
      fetchNextPage();
    });
  ...
  if (existingRecords.length > 0) {
    ...
    for (let i = 0; i < existingRecords.length; i += batchSize) {
      ...
      await base('Outcome-Worldview Relationships').destroy(batch);
      console.log(`  Deleted ${Math.min(i + batchSize, existingRecords.length)}/${existingRecords.length}`);
    }
